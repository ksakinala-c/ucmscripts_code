				...
				'callback' => function(&$message){
					?> <img src="<?php echo _BASE_HREF;?>includes/plugin_social_twitter/images/twitter-logo.png" class="twitter_icon">
					    <a href="<?php echo $message['twitter_message']->get_link(); ?>"
				           target="_blank"><?php echo htmlspecialchars( $message['twitter_message']->get('twitter_account')->get( 'account_name' ) ); ?></a> <br/>
				...
				'callback' => function(&$message){
				    ...
				    <div class="social_from_full">
						...
						foreach($from as $id => $from_data){
							?>
							<div>
								<a href="http://twitter.com/<?php echo htmlspecialchars($from_data['screen_name']);?>" target="_blank"><img src="<?php echo $from_data['image'];?>" class="social_from_picture"></a> <?php echo htmlspecialchars($from_data['screen_name']); ?>
							</div>
							<?php
						} ?>
				    </div>
			        ...
			        <?php if(module_social::can_i('edit','Twitter Comments','Social','social')){ ?>
					    <?php if($message['twitter_message']->get('status') == _SOCIAL_MESSAGE_STATUS_ANSWERED){  ?>
						    <a href="#" class="socialtwitter_message_action btn btn-default btn-xs"
						       data-action="set-unanswered" data-id="<?php echo (int)$message['twitter_message']->get('social_twitter_message_id');?>" data-social_twitter_id="<?php echo (int)$message['twitter_message']->get('social_twitter_id');?>"><?php _e( 'Un-Archive' ); ?></a>
					    <?php }else{ ?>
						    <a href="#" class="socialtwitter_message_action btn btn-default btn-xs"
						       data-action="set-answered" data-id="<?php echo (int)$message['twitter_message']->get('social_twitter_message_id');?>" data-social_twitter_id="<?php echo (int)$message['twitter_message']->get('social_twitter_id');?>"><?php _e( 'Archive' ); ?></a>
					    <?php } ?>
			        <?php }
				},
			);
