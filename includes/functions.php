...
function print_multi_select_box($data,$id,$cur=array(),$blank=true,$array_id=false,$hidden=true){
    ...
    <?php
        ...
        if($hidden)echo '<div style="overflow-y: auto; width:220px; height:300px; position: absolute; background: #FFF; font-size:0.9em; display:none;">';
        $x=0;
        foreach($data as $key => $val){
            if(is_array($val)){
                if(!$array_id){
            ...
            if(strlen($printval)==0)continue;
            ?>
            <input type="checkbox" name="<?php echo $id;?>[]" value="<?php echo $key;?>" id="multi_<?php echo $x;?>" <?php echo in_array($key,$cur) ? ' checked' : '';?>>
...
function hook_add($hook_name,$callback){
    global $hooks;
    if(!isset($hooks[$hook_name]))$hooks[$hook_name]=array();
...
function _h($text){
    // is help enabled?
    if(!module_config::c('show_help',1) || get_display_mode() == 'mobile')return '';
    $help_id = md5($text);
    $argv = func_get_args();
    ?> <a href="#" onclick="ucm.open_help('<?php echo $help_id;?>');return false;" class="ui-corner-all ui-icon ui-icon-help help-icon" style="display:inline-block;">[?]</a>
...
function forum_text($original_text, $htmlspecialchars=true){
	...
	foreach($lines as $line_number => $line){
		...
		$print_text .= $line;
        if(($line_number+1)<count($lines)) $print_text .= "<br />\n";
	}
...
function popup_link($link,$options=array()){
    ...
    if(isset($options['force']) && $options['force']){
        ...
        echo $link;
        ?>
        <a href="#" onclick="$('#popup_link<?php echo $hash;?>').dialog('open'); return false;">(<?php _e('popup');?>)</a>
    <?php } ?>
    <div id="popup_link<?php echo $hash;?>" title="<?php echo isset($options['title']) ? htmlspecialchars($options['title']) : '';?>">
