...
define('_CUSTOM_DATA_HOOK_LOCATION_CUSTOMER_FOOTER',9); // inspect the select element on Custom Data page to see used ID's
...
', 'Customer Signup Form', array() );
    ...
    public static function get_current_customer_type_id(){
	    if(isset($_GET['customer_type_id']))return (int)$_GET['customer_type_id'];
    ...
    /** static stuff */
     ...
     public static function link_generate($customer_id=false,$options=array(),$link_options=array()){
        ...
        // generate the arguments for this link
	     if(!isset($options['type']))$options['type']='customer';
	     if(!isset($options['page']))$options['page']='customer_admin_' . (($customer_id||$customer_id=='new') ? 'open' : 'list');
	...
	public static function link_open_customer_type($customer_type_id,$full=false, $data=array()){
		return self::link_generate($customer_type_id,array('page'=>'customer_settings_types','full'=>$full,'data'=>$data,'arguments'=>array('customer_type_id'=>$customer_type_id)));
	}
...
//                }
    ...
    public static function update_customer_status($customer_id,$debug=false){
        ...
        if(class_exists('module_invoice',false)){
	        if($debug)echo "Updating customer status of $customer_id ... ";
            ...
            if($debug)echo "$total_invoices. invoices, $total_overdue overdue, $total_due due ";
            if($total_overdue>0){
	...
	public function save_customer($customer_id,$data){
        ...
        if(isset($data['primary_user_id']))unset($data['primary_user_id']); // only allow this to be set through the method.
    ...
    public static function handle_import($data,$add_to_group,$options=array()){
        ...
        foreach($data as $rowid => $row){
            ...
            if(isset($row['primary_user_name']))$user_update['name'] = $row['primary_user_name'];
            if(isset($row['primary_user_last_name']))$user_update['last_name'] = $row['primary_user_last_name'];
            if(isset($row['primary_user_email']))$user_update['email'] = $row['primary_user_email'];
            if(isset($row['primary_user_phone']))$user_update['phone'] = $row['primary_user_phone'];
            if(isset($row['primary_user_fax']))$user_update['fax'] = $row['primary_user_fax'];
            if(isset($row['primary_user_mobile']))$user_update['mobile'] = $row['primary_user_mobile'];
            if(isset($row['primary_user_language']))$user_update['language'] = $row['primary_user_language'];
            if(isset($row['password']) && strlen($row['password']))$user_update['password'] = md5(trim($row['password']));
    ...
    public static function hook_filter_var_customer_list($call, $customer_attributes){
        if(!is_array($customer_attributes))$customer_attributes = array();
    ...
    public function external_hook($hook){
        ...
        switch($hook){
            ...
            case 'public_signup':
                ...
                $allowed = array('name','last_name','customer_name','email','phone','mobile','extra','customer_type_id');
                ...
                if($job && isset($job['type']) && is_array($job['type'])){
                    if(module_config::c('customer_signup_any_job_type',0)){
                        foreach($job['type'] as $type_name){
                            ...
                            if(!$job_data['name'])$job_data['name'] = $type_name;
                ...
                if(!count($subscription['subscription_name']))$subscription['subscription_name'][] = _l('N/A');
                if(!count($subscription['subscription_invoice']))$subscription['subscription_invoice'][] = _l('N/A');
                ...
                $values['CUSTOMER_LINK'] = module_customer::link_generate($customer_id,array('skip_permissions'=>true,'full'=>false,'data'=>$customer_data));
                //$values['CUSTOMER_NAME_LINK'] = module_customer::link_open($customer_id,true,$customer_data);
                $values['CUSTOMER_NAME_LINK'] = module_customer::link_generate($customer_id,array('skip_permissions'=>true,'full'=>true,'data'=>$customer_data));
                if($website_id){
	            ...
	            if(isset($_REQUEST['customer_signup_email_admin_template']))$customer_signup_template = $_REQUEST['customer_signup_email_admin_template'];
				...
				if(isset($_REQUEST['customer_signup_email_welcome_template']))$customer_signup_template = $_REQUEST['customer_signup_email_welcome_template'];
				if($customer_signup_template) {
					$template = module_template::get_template_by_key( $customer_signup_template );
					if($template->template_id) {
						$template->assign_values( $values );
	...
	public static function dynamic_customer_selection($customer_id = false, $input_id = 'customer_id'){
		...
		<div class="dynamic_customer_selection <?php echo $customer_id ? ' has-current' : '';?>">
			...
			<div class="choose_customer">
				<input type="hidden" name="<?php echo $input_id;?>" value="<?php echo (int)$customer_id;?>" class="change_customer_id_input">
  ...
  PRIMARY KEY  (`customer_id`, `user_id`)
		...
		PRIMARY KEY  (`customer_type_id`)
			...
			$sql .= 'INSERT INTO `'._DB_PREFIX.'customer_type` VALUES (1, 0, \'users\', \'Lead\', \'Leads\', NOW(), NOW());';
	...
	public function get_install_sql(){
		...
		INSERT INTO `<?php echo _DB_PREFIX; ?>customer` VALUES (1, 0, 3, 0, 'Bobs Printing Service', 0, -1, '', '', NOW(), NOW());
		INSERT INTO `<?php echo _DB_PREFIX; ?>customer` VALUES (2, 0, 4, 0, 'Richards Roof Repairs', 0, -1, '', '', NOW(), NOW());
		...
		INSERT INTO `<?php echo _DB_PREFIX; ?>customer_type` VALUES (1, 0, 'users', 'Lead', 'Leads', NOW(), NOW());
