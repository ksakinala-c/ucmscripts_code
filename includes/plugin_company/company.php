...
class module_company extends module_base{
    ...
    public static function get_companys_access_restrictions(){
        if(self::$get_companys_access_restrictions_cache!==false)return self::$get_companys_access_restrictions_cache;
        $where = 'WHERE 1';
        ...
        if(defined('COMPANY_UNIQUE_CONFIG') && COMPANY_UNIQUE_CONFIG){
            ...
            if((int)$company_id>0){
                ...
                if($company && $company['company_id'] == $company_id){
                    $data = get_single("company_template",array('company_id','template_id'),array($company_id,$template_id));
