	...
	public static function display_extras($options){
		...
		if($owner_table){
			...
			foreach($extra_items as $extra_item){
				...
				ob_start();
                ?>
                <tr id="extra_<?php echo $extra_id;?>" class="extra_field_row<?php echo !empty($extra_item['hidden']) ? ' extra_field_row_hidden' : '';?>">
            ...
            if($more_fields_available || $allow_new) {
	            ...
	            if($allow_new){
		            ...
		            <tr id="extra_<?php echo $extra_id;?>" class="extra_field_row_hidden">
			            <th>
				            <input type="text" name="extra_<?php echo $owner_table;?>_field[<?php echo $extra_id;?>][key]" value="<?php ?>" class="extra_field" placeholder="<?php _e('New Field');?>">
			            </th>
			            <td>
				            <input type="text" name="extra_<?php echo $owner_table;?>_field[<?php echo $extra_id;?>][val]" value="<?php ?>" placeholder="<?php _e('New Value');?>">
    ...
    public static function print_table_header($owner_table,$options=array())
    {
        ...
        if(self::can_i('view','Extra Fields')){
            ...
            foreach($column_headers as $column_header){
                ...
                ?>
                <th class="extra_column" id="extra_header_<?php echo $column_header['extra_default_id'];?>" <?php echo isset($this_options['style']) ? ' style="'.$this_options['style'].'"' : '';?>>
