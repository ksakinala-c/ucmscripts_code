...
class module_paymethod_stripe extends module_base{
	...
	public function is_allowed_for_invoice($invoice_id){
	    ...
	    if(isset($invoice_payment_methods['stripe']))return $invoice_payment_methods['stripe']['enabled'];
    ...
    public static function start_payment($invoice_id,$payment_amount,$invoice_payment_id,$user_id=false){
        if($invoice_id && $payment_amount && $invoice_payment_id){
            ...
            if(!$user_id)$user_id = isset($invoice_data['primary_user_id']) ? $invoice_data['primary_user_id'] : 0;
                        ...
                        if($stripe_plan){
                            ...
                            $template = new module_template();
                             ob_start();
                                ...
                                <form action="<?php echo full_link(_EXTERNAL_TUNNEL.'?m=paymethod_stripe&h=pay_subscription&method=stripe');?>" method="post">
                                    <input type="hidden" name="invoice_payment_subscription_id" value="<?php echo $invoice_payment_subscription_id;?>">
...
echo $template->render('pretty_html');
    ...
    public function is_installed(){ return true; }
    public function get_install_sql() { return $this->get_upgrade_sql(); }
