...
?>
    ...
    <p>This report will show an overview of the taxible income and expenditure from items recorded in the "Transactions" area.</p>
...
<tr class="title">
	...
	<?php foreach($currencies as $currency_id){
		...
		if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'i'){ ?>
			<th colspan="<?php echo 2 + (isset($available_credit_taxes[$currency_id]) ? count($available_credit_taxes[$currency_id]) : 0);?>"><?php echo $currency && isset($currency['code']) ? $currency['code'] : ''?> <?php _e('Income/Credit');?></th>
	    <?php } ?>
	    <?php if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'e'){ ?>
	        <th colspan="<?php echo 2 + (isset($available_debit_taxes[$currency_id]) ? count($available_debit_taxes[$currency_id]) : 0);?>"><?php echo $currency && isset($currency['code']) ? $currency['code'] : ''?> <?php _e('Expense/Debit');?></th>
	    <?php } ?>
	<?php } ?>
...
<tr>
	<?php foreach($currencies as $currency_id){
		...
		if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'i'){ ?>
			...
			<?php if(isset($available_credit_taxes[$currency_id]))foreach($available_credit_taxes[$currency_id] as $tax_percent => $tf){ ?>
	    ...
	    <?php if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'e'){ ?>
			...
			<?php if(isset($available_debit_taxes[$currency_id]))foreach($available_debit_taxes[$currency_id] as $tax_percent => $tf){ ?>
							...
							foreach($finance_record['taxes'] as $tax){
								if($finance['credit'] > 0 && $tax['percent'] == $tax_percent){
									if($tax['amount'] > 0 || $tax['percent'] > 0){
		                                if(!$tax['name'])$tax['name'] = _l('N/A');
		                                if(!$tax['percent'])$tax['percent'] = 0;
		...
		if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'i'){ ?>
			...
			<td>
			    <?php if(isset($total_by_currency[$currency_id]['total_credit'])){
				    ?>
                <span class="success_text">+<?php echo dollar($total_by_currency[$currency_id]['total_credit'],true,$currency_id);?></span>
				<?php } ?>
            </td>
	    <?php }
		if(!isset($search['type']) || $search['type'] == 'ie' || $search['type'] == 'e'){ ?>
			...
			<td>
			    <?php if(isset($total_by_currency[$currency_id]['total_debit'])){
				    ?>
                <span class="error_text">-<?php echo dollar($total_by_currency[$currency_id]['total_debit'],true,$currency_id);?></span>
				<?php } ?>
            </td>
	    <?php } ?>
	<?php } ?>
