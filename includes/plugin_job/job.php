...
//                        $jobs = qa($sql);
	...
	public static function link_create_job_invoice($job_id,$full=false){
        return self::link_generate($job_id,array('full'=>$full,'arguments'=>array('_process'=>'ajax_create_invoice')));
    }
...
','Used when displaying the external view of a job.','code');
	...
	public function process(){
		...
		if(isset($_REQUEST['butt_del']) && $_REQUEST['butt_del'] && $_REQUEST['job_id']){
            ...
            if(!$job || $job['job_id'] != $job_id)exit; // no permissions.
            if(!module_invoice::can_i('create','Invoices'))exit; // no permissions
                ...
                <ul>
                    <?php foreach($job['uninvoiced_task_ids'] as $task_id){
                        if(isset($job_tasks[$task_id])){
                            ?>
                            <li>
                                <input type="checkbox" id="invoice_create_task_<?php echo $task_id;?>" data-taskid="<?php echo $task_id;?>" class="invoice_create_task" name="invoice_task_id[<?php echo $task_id;?>]" value="1" <?php echo $job_tasks[$task_id]['fully_completed'] ? 'checked':'';?>>
...
//		return get_multiple("job",$search,"job_id","fuzzy","name");
    ...
    public static function get_tasks($job_id,$order_by='task'){
	    ...
	    if(isset(self::$job_tasks_cache[$job_id]))return self::$job_tasks_cache[$job_id];
...
//        print_r($res);exit;
            ...
            foreach($tax_rates as $tax_rate_id => $tax_rate_amount){
                if($tax_rate_amount > 0){
                    $job['taxes'][] = array(
                        ...
                        'amount' => 0, // final amount of calculated tax
                ...
                foreach($tasks as $task_id => $task){
                    ...
                    if(self::job_task_has_split_hours($job_id,$job,$task_id,$task)){
                        ...
                        if($task['billable']){
                            ...
                            if(!isset($job['staff_total_grouped'][$task['user_id']]))$job['staff_total_grouped'][$task['user_id']]=0;
		    ...
		    if($task_data['user_id'] > 0 && $task_data['user_id'] != module_security::get_loggedin_id()){
			    ...
			    if($staff && $staff['user_id'] == $task_data['user_id'] && !(module_config::c('job_staff_email_skip_complete',0) && $task_data['fully_completed'])){
					...
					//foreach($job_tasks as $job_task){
					    ...
					    if($job_task['fully_completed']) $job_data['job_tasks'] .= ' <span style="color: #99cc00; font-weight:bold;">('._l('complete').')</span>';
					    $job_data['job_tasks'] .= ' <br/>';
					    if($job_task['long_description'])$job_data['job_tasks'] .= _l('Notes:').' <em>'.$job_task['long_description'].'</em><br/>';
					    if($job_task['date_due']&&$job_task['date_due']!='0000-00-00')$job_data['job_tasks'] .= _l('Date Due:').' '.print_date($job_task['date_due']).'<br/>';
					    if($job_task['hours'])$job_data['job_tasks'] .= _l('Assigned Hours:').' '.$job_task['hours'].'<br/>';
					    if($job_task['completed'])$job_data['job_tasks'] .= _l('Completed Hours:').' '.$job_task['completed'].'<br/>';
		    ...
		    if($task_data['user_id'] > 0){
			    ...
			    if($staff && $staff['user_id'] == $task_data['user_id'] && !(module_config::c('job_staff_email_skip_complete',0) && $task_data['fully_completed'])){
					...
					//foreach($job_tasks as $job_task){
					    ...
					    if($job_task['fully_completed']) $job_data['job_task'] .= ' <span style="color: #99cc00; font-weight:bold;">('._l('complete').')</span>';
					    $job_data['job_task'] .= ' <br/>';
					    if($job_task['long_description'])$job_data['job_task'] .= _l('Notes:').' <em>'.$job_task['long_description'].'</em><br/>';
					    if($job_task['date_due']&&$job_task['date_due']!='0000-00-00')$job_data['job_task'] .= _l('Date Due:').' '.print_date($job_task['date_due']).'<br/>';
					    if($job_task['hours'])$job_data['job_task'] .= _l('Assigned Hours:').' '.$job_task['hours'].'<br/>';
					    if(isset($job_task['completed']) && $job_task['completed'])$job_data['job_task'] .= _l('Completed Hours:').' '.(isset($job_task['completed']) ? $job_task['completed'] : '').'<br/>';
	...
	private static function save_job_cache($job_id, $data = array()){
        if(!$data)$data = self::get_job($job_id);
    ...
    public function run_cron($debug=false){
        ...
        if($time_of_day < $after_time){
            if($debug)echo "Not performing automatic invoice operations until after $after_time:00 - it is currently $time_of_day:".date('i')."<br>\n";
            return;
        }
        ...
        foreach($renew_jobs as $renew_job){
            ...
            if($debug)echo "Automatically Renewing Job ".module_job::link_open($renew_job['job_id'],true) ."<br>\n";
            ...
            if(module_config::c('invoice_auto_renew_only_paid_invoices',1) && $unpaid_invoice){
                if($debug)echo "Not automatically renewing this job because it has unpaid invoices. <br>\n";
            }else{
                $new_job_id = $this->renew_job($renew_job['job_id'], true);
                if($new_job_id){
                    //module_cache::clear_cache();
                    if($debug)echo "Job Automatically Renewed: ".module_job::link_open($new_job_id,true)."<br>\n";
                    if($renew_job['renew_invoice']){
                        ...
                        //module_cache::clear_cache();
                        if($debug)echo "Generated new invoice for renewed job: ".module_invoice::link_open($new_invoice_id,true) ."<br/>";
                        if($debug)echo "Emailing invoice to customer...";

                        if(module_invoice::email_invoice_to_customer($new_invoice_id)){
                            if($debug)echo "send successfully";
                        }else{
                            if($debug)echo "send failed";
                        }
                        if($debug)echo "<br>\n";
    ...
    public static function hook_filter_var_job_list($call, $attributes){
        ...
        foreach(module_job::get_jobs(array(
