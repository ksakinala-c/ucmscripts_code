...
if(!$job['hourly_rate'])$job['hourly_rate'] = 0;
	    ...
	    $fieldset_data = array(
		    ...
		    'elements'       => array(
			    array(
				    ...
				    'field' => array(
					    'type' => 'html',
					    'value' => '<a href="' . module_job::link_public( $job_id ) . '" target="_blank">' . _l( 'Click to view external link' ) . '</a>',
					    'help' => 'You can send this link to the customer and they can view progress on their job. They can also view a list of any invoices attached to this job. This is VERY useful to stop customers asking you "how far along are you" with a job because they can see exactly where you have logged up to in the system.',
				    ),
			    ),
		    ),
	    );
                ...
                <?php if(module_job::can_i('edit','Job Tasks')||module_job::can_i('view','Job Tasks')){
                    ...
                    <table border="0" cellspacing="0" cellpadding="2" class="tableclass tableclass_rows tableclass_full" id="job_task_listing">
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <td valign="top">
                                <input type="text" name="job_task[new][description]" id="task_desc_new" class="edit_task_description no_permissions" value=""><?php
                            ...
                            <td align="center" valign="top">
                                <input type="submit" name="save" value="<?php _e('New Task');?>" class="save_task no_permissions small_button">
                        ...
                        <?php if((int)$job_id>0){
                            ?> <div id="job_summary"> <?php echo module_job::generate_job_summary($job_id,$job,$show_hours_summary); ?> </div> <?php
                        } ?>
                            </div>
