...
class module_base{
	...
	public static function can_i($actions,$name=false,$category=false,$module=false){
		...
		if(!$name)$name = ucwords(str_replace('_',' ',str_replace('module_','',$class_name)));
        if(!$name)return false;
        if(!$category)$category = ucwords(str_replace('_',' ',str_replace('module_','',$class_name)));
		$perms = array(
				'name' => $name,
    ...
    public static function is_plugin_enabled(){
        if(!_UCM_INSTALLED)return false;
        $class_name = false;
        if(function_exists('get_called_class')){
            $class_name = get_called_class();
        }else if(is_callable('self::get_class()')){
	...
	function get_menu($holding_module=false,$holding_page=false,$type=false){
		...
		foreach($links as $link_id => $link){
			...
			if(!$order)$order = isset($link['order']) ? $link['order'] : $this->module_position;
	...
	public static function db_table_exists($name,$force=false){
        if(defined('_UCM_INSTALLED') && !_UCM_INSTALLED)return false;
        if($force) {
            if(isset(self::$_dbt_exists[$name]) && self::$_dbt_exists[$name])return true;
