		...
		<form action="" method="post">
				...
				'callback' => function(&$message){
				    ...
				    <div class="social_from_full">
						...
						foreach($from as $id => $name){
							?>
							<div>
								<a href="http://facebook.com/<?php echo $id;?>" target="_blank"><img src="http://graph.facebook.com/<?php echo $id;?>/picture" class="social_from_picture"></a> <?php echo htmlspecialchars($name); ?>
							</div>
							<?php
						} ?>
				    </div>
			...
			$columns['social_column_action'] = array(
				...
				'callback' => function(&$message){
					if(module_social::can_i('view','Facebook Comments','Social','social')){
						?>
				        <a href="<?php echo module_social_facebook::link_open_facebook_message($message['social_facebook_id'],$message['social_facebook_message_id']);?>" class="socialfacebook_message_open social_modal btn btn-default btn-xs" data-modal-title="<?php echo htmlspecialchars($message['facebook_message']->get_summary());?>"><?php _e( 'Open' );?></a>
			        <?php } ?>
			        <?php if(module_social::can_i('edit','Facebook Comments','Social','social')){ ?>
					    <?php if($message['facebook_message']->get('status') == _SOCIAL_MESSAGE_STATUS_ANSWERED){  ?>
						    <a href="#" class="socialfacebook_message_action  btn btn-default btn-xs"
						       data-action="set-unanswered" data-id="<?php echo (int)$message['facebook_message']->get('social_facebook_message_id');?>"><?php _e( 'Un-Archive' ); ?></a>
					    <?php }else{ ?>
						    <a href="#" class="socialfacebook_message_action  btn btn-default btn-xs"
						       data-action="set-answered" data-id="<?php echo (int)$message['facebook_message']->get('social_facebook_message_id');?>"><?php _e( 'Archive' ); ?></a>
					    <?php } ?>
			        <?php }
				},
			);
