...
abstract class BaseFacebook
{
  ...
  public function getClientCode($long_lived_access_token) {
    ...
    try {
      // need to circumvent json_decode by calling _oauthRequest
      // directly, since response isn't JSON format.
      $client_code_response =
        $this->_oauthRequest(
          $this->getUrl('graph', '/oauth/client_code'),
          $params = array(
                          'client_secret' => $this->getAppSecret(),
                          'redirect_uri' => $redirect_uri,
    ...
    if (empty($client_code_response)) {
      return false;
    }

    $response_params = array();
    parse_str($client_code_response, $response_params);
