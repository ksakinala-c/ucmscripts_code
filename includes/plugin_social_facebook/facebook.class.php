		...
		<tr class="<?php echo isset($settings['row_class']) ? $settings['row_class'] : '';?> facebook_message_row <?php echo !isset($message['read_time']) || !$message['read_time'] ? ' message_row_unread' : '';?>"
		    ...
		    <td>
			    ...
			    <a href="<?php echo $facebook_message->get_link(); ?>"
		           target="_blank"><?php echo htmlspecialchars( $facebook_message->get('facebook_page')->get( 'page_name' ) ); ?></a> <br/>
		    ...
		    <td nowrap>
		        <?php if(module_social::can_i('view','Facebook Comments','Social','social')){ ?>
			        ...
			        <a href="<?php echo module_social_facebook::link_open_facebook_message($message['social_facebook_id'],$message['social_facebook_message_id']);?>" class="socialfacebook_message_open social_modal btn btn-default btn-xs" data-modal-title="<?php echo htmlspecialchars($summary);?>"><?php _e( 'Open' );?></a>
		        ...
		        <?php if(module_social::can_i('edit','Facebook Comments','Social','social')){ ?>
				    <?php if($facebook_message->get('status') == _SOCIAL_MESSAGE_STATUS_ANSWERED){  ?>
					    <a href="#" class="socialfacebook_message_action  btn btn-default btn-xs"
					       data-action="set-unanswered" data-id="<?php echo (int)$facebook_message->get('social_facebook_message_id');?>"><?php _e( 'Un-Archive' ); ?></a>
				    <?php }else{ ?>
					    <a href="#" class="socialfacebook_message_action  btn btn-default btn-xs"
...
class ucm_facebook_page{
	...
	public function graph_load_latest_page_data($debug = false){
		...
		if($debug)echo "The last message we received for this page was on: ".print_date($last_message_received,true).'<br>';
		...
		if(isset($page_feed['data']) && !empty($page_feed['data'])){
			foreach($page_feed['data'] as $page_feed_message){
				...
				if($last_message_received && $last_message_received >= $message_time){
					...
					if($debug)echo " - Skipping this message because it was received on ".print_date($message_time,true).' and we only want ones after '.print_date($last_message_received,true).'<br>';
					break;
				}else{
					if($debug)echo ' - storing message received on '.print_date($message_time,true).'<br>';
				}
		...
		if($debug)echo "The last message we received for this page was now on: ".print_date($newest_message_received,true).'<br>';
		if($debug)echo "Finished checking this page messages at ".print_date(time(),true)."<br>";
...
class ucm_facebook_message{
	    ...
	    if($data && isset($data['data']) && is_array($data['data'])){
		    foreach($data['data'] as $comment){
			    if($comment['id']){
				    ...
				    $exists = get_single('social_facebook_message_comment',array('facebook_id','social_facebook_message_id'),array($comment['id'],$this->social_facebook_message_id));
	...
	public function reply_box($facebook_id,$level=1){
		if($this->facebook_account && $this->facebook_page && $this->social_facebook_message_id) {
			...
			<div class="facebook_comment facebook_comment_reply_box facebook_comment_reply_box_level<?php echo $level;?>">
				...
				<div class="facebook_comment_header">
					<?php echo ucm_facebook::format_person( array('id'=>$this->facebook_page->get('page_id'), 'name'=>$this->facebook_page->get('page_name')) ); ?>
				</div>
	...
	public function send_queued($debug = false){
		if($this->facebook_account && $this->facebook_page && $this->social_facebook_message_id) {
			...
			if($debug)echo "Sending a new message to facebook Page ID: $facebook_page_id <br>\n";
			$result = false;
			$facebook = new ucm_facebook();
			$post_data = array(
				'access_token' => $access_token,
