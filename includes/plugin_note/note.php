...
class module_note extends module_base{
	...
	public static function process_note_permissions($alert_res, $task){
		switch($task['owner_table']){
			...
			case 'invoice':
				...
				if(!$invoice_data||!isset($invoice_data['invoice_id'])||$invoice_data['invoice_id']!=$task['owner_id'])return false;
			...
			case 'quote':
				...
				if(!$quote_data||!isset($quote_data['quote_id'])||$quote_data['quote_id']!=$task['owner_id'])return false;
			...
			case 'website':
				...
				if(!$website_data||!isset($website_data['website_id'])||$website_data['website_id']!=$task['owner_id'])return false;
			...
			case 'customer':
				...
				if(!$customer_data||!isset($customer_data['customer_id'])||$customer_data['customer_id']!=$task['owner_id'])return false;
			...
			case 'vendor':
				...
				if(!$vendor_data||!isset($vendor_data['vendor_id'])||$vendor_data['vendor_id']!=$task['owner_id'])return false;
			...
			case 'job';
				...
				if(!$job_data||!isset($job_data['job_id'])||$job_data['job_id']!=$task['owner_id'])return false;
	...
	public static function print_note($note_id,$note_item,$display_summary=false,$can_edit=true,$can_delete=true,$options=array()){
		...
		if(isset($options['summary_owners']))unset($options['summary_owners']);
		if(isset($options['display_summary']))unset($options['display_summary']);
