...
// for windows hosting:
    ...
    ?>
    <div class="warning">
        Warning, the hosting provider is using the suhosin patch for PHP, which limit the maximum number of fields to post in a form:
        <?php echo $sohusin1;?> for suhosin.post.max_vars. <br/>
        <?php echo $sohusin2;?> for suhosin.request.max_vars. <br/>
        Please ask your hosting provider to increase the suhosin post and request limit to <?php echo $post_field_count;?> at least. Or import less rows at a time (eg: 20 records at a time).
    </div>
    <?php }
    ...
    <div class="warning">
        ...
        Please ask your hosting provider to increase the 'max_input_vars' setting to <?php echo $post_field_count;?> or higher. Or import less rows at a time (eg: 20 records at a time).
    </div>
    <?php } ?>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <?php if(isset($import_options['options'])&&is_array($import_options['options'])){
            foreach($import_options['options'] as $option_id => $option_values){
            ?>
            <input type="hidden" name="<?php echo htmlspecialchars($option_values['form_element']['name']);?>" value="<?php echo htmlspecialchars(isset($_REQUEST[$option_values['form_element']['name']])?$_REQUEST[$option_values['form_element']['name']]:'');?>">
            <?php }
        }?>
        ...
        <p style="color:#FF0000"><?php _e('We highly recommend splitting the import file up into smaller files!');?></p>
        <?php } ?>
            ...
            <tr>
                ...
                <th><select name="c[<?php echo $column;?>]">
                    ...
                    <?php foreach($import_options['fields'] as $key2=>$val2){
                        ...
                        $this_key = $val2[0];
                        ?>
                        <option value="<?php echo $this_key;?>"<?php if(strtolower($display_key)==strtolower($key2)){ echo ' selected'; $column_matches[$column] = $this_key; }?>><?php echo htmlspecialchars($key2);?></option>
                    <?php }
                    foreach($extra_fields as $fields){
                        foreach($fields as $key=>$val){
                            ...
                            $key2 = $val['extra_key'];
                            ?>
                            <option value="<?php echo $this_key;?>"<?php echo strtolower($display_key)==strtolower($key2) ? ' selected':'';?>><?php echo htmlspecialchars($key2);?></option>
                            <?php
                        }
                    }
                    ?>
            ...
            <?php
                ...
                <tr class="<?php echo $rowid%2?'odd':'even';?>" rel="<?php echo $rowid;?>">
                    ...
                    <td>
                        ...
                        <input type="hidden" name="fullrow[<?php echo $rowid;?>]" value="<?php echo htmlspecialchars(implode(_CSV_JS_DELIM,$row));?>">
                    </td>
                </tr>
            <?php
            $rowid++;
