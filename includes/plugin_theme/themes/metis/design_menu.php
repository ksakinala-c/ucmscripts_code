...
//        $link['name'] = str_replace('menu_label','badge',$link['name']);
        ...
        ob_start();
        ?>
        <li class="<?php if($current || !empty($possible_child_items['has_active'])){ echo ' active'; } ?>">
            <a href="<?php echo $link_url; ?>"><?php if(($menu_holder == 'main' || $menu_holder == 'customer_admin_open') && isset($link['icon_name'])){ ?><i class="fa fa-<?php echo $link['icon_name'];?>"></i><?php } ?>
        ...
        $menu_return['nav_items_mini'] .= $nav_item;
