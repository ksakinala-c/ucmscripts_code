    ...
    if((isset($options['main']) && $options['main'])){ //} || !isset($GLOBALS['metis_main_title']) || !$GLOBALS['metis_main_title']){
        ...
        <div class="header_buttons">
            <?php foreach($buttons as $button){ ?>
                <a href="<?php echo $button['url'];?>" class="btn btn-metis-5 btn-sm<?php echo isset($button['class'])?' '.$button['class']:'';?>"<?php if(isset($button['id'])) echo ' id="'.$button['id'].'"';?><?php if(isset($button['onclick'])) echo ' onclick="'.$button['onclick'].'"';?>>
