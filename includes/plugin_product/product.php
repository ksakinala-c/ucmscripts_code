...
//                        print_r($line);
	...
	public function save_product($product_id,$data){
		...
		if(!empty($data['inventory_control'])){
			...
			if((int)$product_id>0) {
				...
				$usages = self::get_product_usage( $product_id );
			...
			$data['inventory_level'] = (!empty($data['inventory_level_current']) ? $data['inventory_level_current'] : 0) + $inventory_usage;
		}
	...
	public static function get_product_usage($product_id){
