...
/*
    ...
    private static function _init_vars($only_load_key=false){
        if(self::$config_vars)return false;
    ...
    public static function c($key,$default=false,$options=array()){
        ...
        if(isset(self::$_c[$key]))return false; // init_vars and save_config can sometimes cause a loop
        self::$_c[$key] = true;
	...
	public static function css_combine($version, $reinit=false){
		...
		if(is_file($cache_file) && filesize($cache_file) > 10 && module_config::c('css_cached_version',$version) == $version){
			...
			self::register_css(false,false,_BASE_HREF . 'temp/' . $cache_name);
			foreach(self::$css_files as $hash=>$css_file_info) {
			...
			if(strlen($css_content)>10){
				...
				$old_files = @glob(_UCM_FOLDER.'temp/cache-*.css');
    ...
    public static function print_js($version=false) {
        ...
        foreach(self::$js_files as $hash=>$js_file_info){
            if(strlen($js_file_info[2])<3){ // url is set to 'true', use the module/file name combo
	            ...
	            ?>
                <script type="text/javascript" language="javascript" src="<?php echo _BASE_HREF . $js_file; echo ($version && strpos('?',$js_file[1])===false) ? '?ver='.$version : ''; ?>"></script> <?php
            }else{ ?>
                <script type="text/javascript" language="javascript" src="<?php echo htmlspecialchars($js_file_info[2]);?>"></script> <?php
            }
        }
        if(function_exists('hook_handle_callback')){
	...
	public static function js_combine($version, $reinit=false){
		...
		if(is_file($cache_file) && filesize($cache_file) > 10 && module_config::c('js_cached_version',$version) == $version){
			...
			if(strlen($js_content)>10){
				...
				// remove other older css files.
				$old_files = @glob(_UCM_FOLDER.'temp/cache-*.js');
				if(is_array($old_files)){
					foreach($old_files as $old_file){
						if(is_file($old_file) && filemtime($old_file) < time() - 172800){ // 2 days
							@unlink($old_file);
    ...
    public function get_install_sql(){
        ...
        INSERT INTO `<?php echo _DB_PREFIX;?>config` SET `key` = '_installation_code', `val` = '9c900e67-0d83-4654-a65d-64710ffc8468';
        <?php /* default email settings from the installer: */ 
 self::$_c['admin_email_address'] = 'wongchihong@gmail.com' 
 ?> 
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'admin_email_address', `val` = 'wongchihong@gmail.com';
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'customer_signup_admin_email', `val` = 'wongchihong@gmail.com';
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'error_email_address', `val` = 'wongchihong@gmail.com';
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'newsletter_default_from_email', `val` = 'wongchihong@gmail.com';
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'newsletter_default_bounce', `val` = 'wongchihong@gmail.com';
REPLACE INTO `<?php echo _DB_PREFIX;?>config` SET `key` = 'ticket_admin_email_alert', `val` = 'wongchihong@gmail.com';
    ...
    public static function check_for_upgrades($requested_plugin='',$get_file_contents=0) {
        ...
        foreach($plugins as $plugin_name => $p){
	        if($requested_plugin && $requested_plugin != $plugin_name)continue;
	        if(!$p->is_plugin_enabled())$p->init();
