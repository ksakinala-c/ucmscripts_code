...
define('_BACKUP_BASE_DIR','includes/plugin_backup/backups/');

class module_backup extends module_base{
	
	public $links;

    public $version = 2.115;
    ...
    public static function link_generate($backup_id=false,$options=array(),$link_options=array()){
        ...
        if(!isset($options['type']))$options['type']='backup';
        if(!isset($options['page']))$options['page']='backup_settings';
    ...
    public static function link_open($backup_id,$full=false){
        return self::link_generate($backup_id,array('full'=>$full,'arguments'=>array('backup_id'=>$backup_id)));
    }
	...
	public function backup_database_tables( $db_table_limit, $sql_file ) {
		...
		foreach ( $tables as $table ) {
			if($db_table_limit && $db_table_limit != $table)continue;
			$sql = "\n\n\n";
			...
			while ( $row = mysqli_fetch_row( $result ) ) {
				if(strpos($table,'config')!==false){
					if($row[0] == 'system_base_href')continue;
					if($row[0] == 'system_base_dir')continue;
				}
				$sql = "";
				...
				if (is_dir($source) && is_dir($source.'includes/plugin_backup/') && is_file($source.'init.php')) {
					...
					if($limit_folder == $source || strpos($limit_folder,$source) === 0){
						...
						if ($zip->open($destination, ZIPARCHIVE::CREATE)) {
							...
							foreach ($files as $file) {
								$name = basename($file);
								if($name == '.' || $name == '..' || $name == '.hg' || $name == '.git' || strpos($file,'.old') !== false)continue;
								if(!$recurisive){
									$file = $limit_folder.$file;
								}
