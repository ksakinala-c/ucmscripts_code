...
class module_debug extends module_base{
	...
	public function init(){
        if(self::$start_time==0)self::$start_time = microtime(true);
        ...
        foreach(self::$debug as $debug_id => $debug){
            ...
            <tr>
                ...
                <td>
                    <?php
                    if(isset($debug['time'])){
                        if($this_time>$time_limit)echo '<span style="color:#ff0000; font-weight:bold;">'.$this_time.'</span>';
                        else echo $this_time;
                    }
                    ?>
                </td>
    ...
    public static function print_footer(){
            ?>
            <div id="system_debug_data" style="<?php echo (!self::$show_debug)?'display:none;':'';?>position: absolute; top:15px;z-index:90000" class="tableclass tableclass_rows">
