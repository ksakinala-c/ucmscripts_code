...
','File Comment: {FILE_NAME} {CUSTOMER_NAME}',array(
	...
	public static function link_open($file_id,$full=false,$bucket_parent_file_id=false){
        return self::link_generate($file_id,array('full'=>$full,'arguments'=>array('bucket_parent_file_id'=>$bucket_parent_file_id)));
    }
	...
	public function print_file($file_id,$layout_type='gallery',$editable=true,$options=array()){
		...
		switch($layout_type){
			...
			<div class="file_<?php echo $file_item['file_id'];?>" style="float:left; width:<?php echo $thumb_width+10;?>px; margin:3px; border:1px solid #CCC; text-align:center;">
	...
	function process(){
		if('plupload' == $_REQUEST['_process']){
			...
			if(isset($file_data['file_url']) && strlen($file_data['file_url'])) {
                ...
                if(!$size)echo file_get_contents($file_data['file_path']);
			}else{
				echo 'Not found';
			}
			exit;
    ...
    function send_file_changed_notice($file_id, $new_file = false, $send_to_admin = false){
        ...
        foreach($alert_users as $alert_user){
	        ...
	        if(!$alert_user['user_id'])continue;
        ...
        if($file_data['file_name'] && preg_match('/\.(\w\w\w\w?)$/',$file_name,$matches)){
            ...
            <div style="width:100%; overflow:auto;" id="file_preview">
                ...
                <div id="pointer_holder" style="position: relative; float: left; height: 0pt;"></div>
                        ...
                        <img src="<?php echo module_file::link_public_view($file_id);?>" alt="<?php _e('Click to add a comment');?>" style="cursor:pointer;"> <!-- style="max-width: 100%" -->
                    </div>
            <?php if(!module_security::is_logged_in() || module_file::can_i('create','File Comments')){ ?>
                    <input type="hidden" name="pointers" id="pointer_save" value="<?php echo $file_data['pointers']; ?>" class="no_permissions">
                <?php } ?>
                ...
                case 'pdf':
                    ...
                    ?>
                    <iframe src="//docs.google.com/gview?url=<?php echo urlencode($embed_url);?>&embedded=true" style="width:100%; height:900px;" frameborder="0"></iframe>
            ...
            case 'download_bucket':
                ...
                if($file_id && $hash){
                    ...
                    if($correct_hash == $hash){
                        ...
                        if(!$size)echo file_get_contents(_FILE_UPLOAD_PATH.$zipName);
...
class ucm_file{
	...
	public function get_data(){
		if(count($this->_get_data_cache))return $this->_get_data_cache;
        $file = false;
        if($this->file_id > 0){
	...
	public function get_type() {
		$file = $this->get_data();
		if(isset($_REQUEST['file_type']))return $_REQUEST['file_type'];
