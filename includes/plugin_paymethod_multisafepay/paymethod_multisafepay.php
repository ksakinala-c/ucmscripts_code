...
class module_paymethod_multisafepay extends module_base{
	...
	public function is_allowed_for_invoice($invoice_id){
	    ...
	    if(isset($invoice_payment_methods['multisafepay']))return $invoice_payment_methods['multisafepay']['enabled'];
    ...
    public static function multisafepay_redirect($description,$amount,$user_id,$payment_id,$invoice_id,$currency_id){
        ...
        if(!$user_id)$user_id = $customer['primary_user_id'];
...
*/
    ...
    //        $msp->merchant['site_code']        = MSP_SITE_CODE;
            ...
            if(!is_array($payment_history_data))$payment_history_data=array();
            if(!isset($payment_history_data['log']))$payment_history_data['log']=array();
