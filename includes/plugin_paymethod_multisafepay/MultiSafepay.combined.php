...
class MultiSafepay {
	...
	function startTransaction(){
		...
		// communication error
		if (!$this->reply_xml) return false;
		...
		if (isset($xml['checkoutdata']['shopping-cart']['items']['item'])){
			...
			foreach($xml['checkoutdata']['shopping-cart']['items']['item'] as $item){
				...
				foreach($item as $k => $v){
					...
					if ($k == 'unit-price') $returnItem['currency'] = $v['currency'];
    ...
    function GetXML() {
		...
		foreach($this->item_arr as $item) {
			...
			if($item->merchant_private_item_data != '') {
				...
				if($item->merchant_item_id != '') $xml_data->Element('merchant-item-id', $item->merchant_item_id);
				if($item->tax_table_selector != '') $xml_data->Element('tax-table-selector', $item->tax_table_selector);
		...
		if($this->edit_cart_url != '') $xml_data->Element('edit-cart-url', $this->edit_cart_url);
		if($this->continue_shopping_url != '') $xml_data->Element('continue-shopping-url', $this->continue_shopping_url);

		if(count($this->shipping_arr) > 0) $xml_data->Push('shipping-methods');
		...
		foreach($this->shipping_arr as $ship) {
			if($ship->type == "flat-rate-shipping" || $ship->type == "merchant-calculated-shipping") {
				...
				if (isset($shipping_restrictions)) {
					...
					if($shipping_restrictions->allowed_restrictions) {
						...
						if($shipping_restrictions->allowed_country_area != "") $xml_data->EmptyElement('us-country-area', array('country-area' => $shipping_restrictions->allowed_country_area));
				...
				if ($ship->type == "merchant-calculated-shipping") {
					...
					if (isset($address_filters)) {
						...
						if($address_filters->allowed_restrictions) {
							$xml_data->Push('allowed-areas');
							if($address_filters->allowed_country_area != "") $xml_data->EmptyElement('us-country-area', array('country-area' => $address_filters->allowed_country_area));
			...
			if(count($this->shipping_arr) > 0) $xml_data->Pop('shipping-methods');
			if($this->request_buyer_phone != "") $xml_data->Element('request-buyer-phone-number', $this->request_buyer_phone);
