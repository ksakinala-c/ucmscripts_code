    ...
    if(module_quote::can_i('view','Import Quote Tasks')){
        $import_tasks_link = module_import_export::import_link(
            array(
                ...
                'fields'=>array(
                    //'Quote Name' => 'quote_name',
                    'Task ID' => array('quote_task_id',false,'The existing system ID for this task. Will overwrite existing task ID. Leave blank to create new task.'),
...
<script type="text/javascript">
    ...
    function refresh_task_preview(quote_task_id,html){
        ...
        var h = 0;
        $('.task_row_'+quote_task_id+'').each(function(){h+=$(this).height();});
...
<iframe id="ajax_task_save" name="ajax_task_save" src="about:blank" style="width:0;height:0;visibility: hidden;" frameborder="0"></iframe>
<form action="" method="post" id="quote_form">
    ...
    if(module_quote::can_i('view','Quote Advanced')) {
	    ...
	    /***** QUOTE ADVANCED *****/
	    $fieldset_data = array(
		    'id' => 'quote_advanced', // used for css and hooks
		    'heading'        => array(
			    'type'  => 'h3',
			    'title' => 'Advanced',
		    ),
	    ...
	    if ( (int) $quote_id > 0 && module_quote::can_i( 'edit', 'Quotes' ) ) {
		    $fieldset_data['elements'][] = array(
			    'title'  => 'Customer Link',
			    'fields' => array(
				    '<a href="' . module_quote::link_public( $quote_id ) . '" target="_blank">' . _l( 'Click to view external link' ) . '</a>',
				    _hr( 'You can send this link to the customer and they can view progress on their quote. They can also view a list of any invoices attached to this quote. This is VERY useful to stop customers asking you "how far along are you" with a quote because they can see exactly where you have logged up to in the system.' )
			    ),
		    );
		    $fieldset_data['elements'][] = array(
			    'title'  => 'Email Staff',
		    ...
		    if($quote['customer_id']){
			    ...
			    $fieldset_data['elements'][] = array(
				    ...
				    'field' => array(
					    ...
					    'value' => $quote['contact_user_id'] > 0 ? $quote['contact_user_id'] : 0,
					    'options' => $c,
					    'blank' => false,
				    ),
			    );
	    ...
	    $fieldset_data['elements'][] = array(
		    ...
		    'fields' => array(
			    function () use ( $quote_id, &$quote ) {
				    ...
				    echo ' ';
				    _h( 'Here you can apply a before tax discount to this quote. You can name this anything, eg: DISCOUNT, CREDIT, REFUND, etc..' );
			    }
		    ),
	    );
	    $fieldset_data['elements'][] = array(
	    ...
	    $fieldset_data['elements'][] = array(
		    ...
		    'field' => array(
			    'type'    => 'select',
			    'options' => module_quote::get_task_types(),
			    'name'    => 'default_task_type',
			    'value'   => isset( $quote['default_task_type'] ) ? $quote['default_task_type'] : 0,
		    ...
		    if ( count( $other_templates ) > 1 ) {
			    $fieldset_data['elements'][] = array(
				    ...
				    'field' => array(
					    ...
					    'name'    => 'quote_template_print',
					    'value'   => $current_template,
					    'help'    => 'Choose the default template for PDF printing and PDF emailing. Name your custom templates quote_pdf_SOMETHING for them to appear in this listing.',
				    ),
			    );
		    }
	    }
    ...
    hook_handle_callback('layout_column_half',2,'65');
                ...
                if(module_quote::can_i('edit','Quote Tasks')||module_quote::can_i('view','Quote Tasks')){
                    ...
                    <table border="0" cellspacing="0" cellpadding="2" class="tableclass tableclass_rows tableclass_full" id="quote_task_listing">
                        <thead>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td valign="top" class="nowrap">
                                <?php if($quote['default_task_type']==_TASK_TYPE_AMOUNT_ONLY){
                                    ...
                                    <input type="text" name="quote_task[new][hours]" value="" size="3" style="width:25px;" class="no_permissions" id="task_hours_new">
                                <?php }else if($quote['default_task_type']==_TASK_TYPE_HOURS_AMOUNT){
                                 ?>
                                    <input type="text" name="quote_task[new][hours]" value="" size="3" style="width:25px;" onchange="setamount(this.value,'new');" onkeyup="setamount(this.value,'new');" class="no_permissions" id="task_hours_new">
                                <?php
                                } ?>
                            </td>
                            ...
                            <td align="center" valign="top">
                                <input type="submit" name="save_ajax_task" value="<?php _e('New Task');?>" formtarget="ajax_task_save" class="save_task no_permissions small_button">
                                ...
                                <input type="hidden" name="quote_task[new][taxable]" value="<?php echo module_config::c('task_taxable_default',1) ? 1 : 0;?>" id="taxable_t_new">
