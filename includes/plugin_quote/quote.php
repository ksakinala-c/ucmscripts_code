...
//                            if(!$quote_data || $quote_data['quote_id']!=$task['quote_id'])continue;
	...
	public static function link_ajax_task($quote_id,$full=false){
        return self::link_generate($quote_id,array('full'=>$full,'arguments'=>array('_process'=>'ajax_task')));
    }
	public static function link_create_quote_invoice($quote_id,$full=false){
        return self::link_generate($quote_id,array('full'=>$full,'arguments'=>array('_process'=>'ajax_create_invoice')));
    }
    ...
    public static function get_replace_fields($quote_id,$quote_data=false){
        ...
        if(!$quote_data)$quote_data = self::get_quote($quote_id);
...
//        $data = array_merge($data,$customer_data,$website_data,$quote_data);
	...
	public function process(){
		...
		if(isset($_REQUEST['butt_del']) && $_REQUEST['butt_del'] && $_REQUEST['quote_id']){
            ...
            if(!$quote || $quote['quote_id'] != $quote_id)exit; // no permissions.
                ...
                <ul>
                    <?php foreach($quote['uninvoiced_quote_task_ids'] as $quote_task_id){
                        if(isset($quote_tasks[$quote_task_id])){
                            ?>
                            <li>
                                <input type="checkbox" id="invoice_create_task_<?php echo $quote_task_id;?>" data-taskid="<?php echo $quote_task_id;?>" class="invoice_create_task" name="invoice_quote_task_id[<?php echo $quote_task_id;?>]" value="1" <?php echo $quote_tasks[$quote_task_id]['fully_completed'] ? 'checked':'';?>>
            ...
            if(!$quote || $quote['quote_id'] != $quote_id)exit; // no permissions.
            if(!self::can_i('edit','Quote Tasks'))exit; // no permissions
...
//		return get_multiple("quote",$search,"quote_id","fuzzy","name");
    ...
    public static function get_tasks($quote_id,$order_by='task'){
        if((int)$quote_id<=0)return array();
	...
	public static function get_quote($quote_id,$full=true,$skip_permissions=false){
        ...
        if(!$quote){
            ...
            if(module_config::c('quote_name_incrementing',0)){
                ...
                do{
                    $quotes = get_multiple('quote',array('name'=>$this_quote_number)); //'customer_id'=>$customer_id,
                    if(!count($quotes)){
...
//            $quote['total_amount_discounted'] = 0;
    ...
    public static function handle_import_row($row, $debug, $add_to_group, $extra_options){
        ...
        if($extra){
            foreach($extra as $extra_key => $extra_val){
                // does this one exist?
                $existing_extra = module_extra::get_extras(array('owner_table'=>'quote','owner_id'=>$quote_id,'extra_key'=>$extra_key));
                $extra_id = false;
    ...
    public static function handle_import_tasks($data,$add_to_group){
        ...
        foreach($data as $rowid => $row){
            ...
            if(!$data[$rowid]['quote_task_id'] && $row['description']){
                ...
                $existing_task = get_single('quote_task',array('quote_id','description'),array($quote_id,$row['description']));
                if($existing_task){
    ...
    `quote_template_print` varchar(255) NOT NULL DEFAULT '',
    `user_id` INT NOT NULL DEFAULT  '0',
