    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" class="tableclass tableclass_form">
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <textarea name="description" rows="4" cols="30" style="width:350px; height: 80px;"><?php echo htmlspecialchars($recurring['description']); ?></textarea>
                </td>
            </tr>
            <tr>
                <th>
                ...
                <td valign="top">
                    ...
                    <input type="text" name="amount" value="<?php echo htmlspecialchars($recurring['amount']);?>" class="currency">
                </td>
            </tr>
            <tr>
                <th>
            ...
            <tr>
                ...
                <td valign="top">
                    <input type="radio" name="type" id="income" value="i"<?php echo $recurring['type'] == 'i' ? ' checked' : '';?>> <label for="income"><?php _e('Income/Credit');?></label> <br/>
                    <input type="radio" name="type" id="expense" value="e"<?php echo $recurring['type'] == 'e' ? ' checked' : '';?>> <label for="expense"><?php _e('Expense/Debit');?></label> <br/>
                </td>
            </tr>
            <tr>
                <th>
            ...
            <tr>
                ...
                <td valign="top">
                    ...
                    foreach($categories as $category){ ?>
                        <input type="checkbox" name="finance_category_id[]" value="<?php echo $category['finance_category_id'];?>" id="category_<?php echo $category['finance_category_id'];?>" <?php echo isset($recurring['category_ids'][$category['finance_category_id']]) ? ' checked' : '';?>>
            ...
            <tr>
                ...
                <td valign="top">
                    ...
                    <select name="recurringtype" onchange="change_recurring_type();" id="recurring_type">
                        ...
                        <option value="onceoff"<?php if($finance_recurring_id&&!$recurring['days']&&!$recurring['months']&&!$recurring['years']) echo ' selected';?>><?php _e('Once Off');?></option>
                    </select>
                </td>
            </tr>
        </tbody>
        <tbody>
            <tr>
                ...
                <td valign="top">
                    <input type="text" name="start_date" value="<?php echo ($recurring['start_date']&&$recurring['start_date']!='0000-00-00') ? print_date($recurring['start_date']) : print_date(time()); ?>" class="date_field" />
                </td>
            </tr>
        </tbody>
        <tbody id="recurring_bits" <?php if($finance_recurring_id&&!$recurring['days']&&!$recurring['months']&&!$recurring['years']) echo ' style="display:none;"';?>>
            <tr>
                <th>
                    <?php echo _l('End Date'); ?>
                </th>
                <td valign="top">
                    <input type="text" name="end_date" id="end_date" value="<?php echo print_date($recurring['end_date']); ?>" class="date_field" />
                </td>
            </tr>
            <tr>
                <th>
                ...
                <td valign="top">
                    <input type="text" name="days" id="days" value="<?php echo $recurring['days']; ?>" style="width:30px;" /> <?php _e('Days'); ?><br/>
                    <input type="text" name="months" id="months" value="<?php echo $recurring['months']; ?>" style="width:30px;" /> <?php _e('Months'); ?><br/>
                    <input type="text" name="years" id="years" value="<?php echo $recurring['years']; ?>" style="width:30px;" /> <?php _e('Years'); ?><br/>
                </td>
            </tr>
        </tbody>
        <tbody>
            ...
            <td>
                ...
                if(isset($recurring['next_due_date'])){
                    ...
                    if(!$recurring['next_due_date'] || $recurring['next_due_date'] == '0000-00-00' || ($recurring['end_date']&&$recurring['end_date']!='0000-00-00' && $next_due > strtotime($recurring['end_date']))){
                        ...
                        ?>
                        <input type="text" name="set_next_due_date" value="<?php echo print_date($recurring['next_due_date']);?>" class="date_field">
                        <?php
                        if ($next_due < time()){
        ...
        <?php if($show_record_button){ ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo module_finance::link_open_record_recurring($recurring['finance_recurring_id']);?>" class="uibutton"<?php  if($days > 10){ ?> onclick="return confirm('<?php echo addcslashes(_l('This transaction is not due for %s days, are you sure you want to record it?',$days),"'");?>');" <?php } ?>><?php _e('Record this transaction');?></a>
