...
?>
	...
	<input type="hidden" name="finance_recurring_id" value="<?php echo $finance_recurring_id; ?>" />
      <input type="hidden" name="_redirect" value="<?php echo $finance['invoice_id'] ? module_invoice::link_open($finance['invoice_id'],false) : '';?>" id="form_redirect" />
	...
	$fieldset_data['elements'][] = array(
	    ...
	    'fields' => array(
		    function() use ($locked,&$finance,$finance_id){
			    if($locked){ echo print_date($finance['transaction_date']); }else{ ?>
			...
			$fieldset_data['elements'][] = array(
			    ...
			    'fields' => array(
				    function() use (&$is_refunds){
                        ...
                        <a href="<?php echo module_finance::link_open('new',false).'&invoice_payment_id='.$r['invoice_payment_id'];?>" class="success_text"><?php _e('%s on %s', ''.dollar($r['amount'],true,$r['currency_id']), print_date($r['date_paid']));?></a>
                        <?php }
				    }
			    ),
		    );
			...
			$fieldset_data['elements'][] = array(
			    ...
			    'fields' => array(
				    function() use (&$refunded){
                        ...
                        <a href="<?php echo module_finance::link_open('new',false).'&invoice_payment_id='.$r['invoice_payment_id'];?>" class="error_text"><?php _e('%s on %s', '-'.dollar($r['amount'],true,$r['currency_id']), print_date($r['date_paid']));?></a>
                        <?php }
				    }
			    ),
		    );
	...
	$fieldset_data['elements'][] = array(
	    ...
	    'fields' => array(
		    function() use (&$finance,$locked){
			    if($locked){ echo $finance['type'] == 'i' ? _l('Income') : _l('Expense'); }else{ ?>
                <input type="radio" name="type" id="income" value="i"<?php echo $finance['type'] == 'i' ? ' checked' : '';?>> <label for="income"><?php _e('Income/Credit');?></label> <br/>
                <input type="radio" name="type" id="expense" value="e"<?php echo $finance['type'] == 'e' ? ' checked' : '';?>> <label for="expense"><?php _e('Expense/Debit');?></label> <br/>
                <?php }
		    }
	    ),
    );
	$fieldset_data['elements'][] = array(
	    ...
	    'fields' => array(
		    function() use (&$finance,$locked){
                ...
                <input type="text" name="sub_amount" id="finance_sub_amount" value="<?php echo htmlspecialchars($finance['sub_amount']);?>" class="currency" autocomplete="off">
                <?php }
		    }
	    ),
    );
	$fieldset_data['elements'][] = array(
	    ...
	    'fields' => array(
		    function() use (&$finance,$locked){
                ...
                <?php echo currency('');?>
                <input type="text" name="taxable_amount" id="finance_taxable_amount" value="<?php echo htmlspecialchars($finance['taxable_amount']);?>" class="currency" autocomplete="off">
                <?php } ?>
                <?php
		    },
		    array(
			    ...
			    if($locked){
                        ...
                        foreach($finance['taxes'] as $id=>$tax){  ?>
                            <div class="dynamic_block">
                                <input type="hidden" name="tax_ids[]" class="dynamic_clear" value="<?php echo isset($tax['finance_tax_id']) ? (int)$tax['finance_tax_id'] : 0;?>">
                                <input type="text" name="tax_names[]" class="dynamic_clear" value="<?php echo isset($tax['name']) ? htmlspecialchars($tax['name']) : '';?>" style="width:30px;" autocomplete="off">
                                @
                                <input type="text" name="tax_percents[]" class="dynamic_clear tax_percent" value="<?php echo isset($tax['percent']) ? htmlspecialchars(number_out($tax['percent'])) : '';?>" style="width:35px;" autocomplete="off">%
                                <input type="hidden" name="tax_amount[]" class="dynamic_clear tax_amount_input" value="<?php echo isset($tax['amount']) ? htmlspecialchars(number_out($tax['amount'])) : '';?>">
		...
		$fieldset_data['elements'][] = array(
		    ...
		    'fields' => array(
			    function() use (&$finance,$locked,$linked_staff_members){
				    ?> <input type="hidden" name="job_staff_expense" value="<?php echo isset($finance['job_staff_expense']) ? (int)$finance['job_staff_expense'] : '';?>">
                    <?php
