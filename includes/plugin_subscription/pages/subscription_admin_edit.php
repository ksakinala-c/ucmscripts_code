				    ...
				    function () use ( &$payment_methods, $subscription ) {
	                    ...
	                    foreach($payment_methods as &$payment_method){
	                        if($payment_method->is_enabled()){
	                            ...
	                            ?>
	                            <input type="checkbox" name="settings[payment_methods][<?php echo $payment_method->module_name;?>]" value="1" id="paymethodallowed<?php echo $x;?>" <?php echo $enabled ? 'checked' : '';?>>
            ...
            foreach($history as $h_id => $h){
                if($h['invoice_id']){
                    ...
                    if($h['paid_date']&&$h['paid_date']!='0000-00-00'){
                        if(!isset($total_paid[$invoice['currency_id']])) $total_paid[$invoice['currency_id']] = 0;
                        $total_paid[$invoice['currency_id']] += $h['amount'];
                    }
                }
                if(!$h['paid_date'] || $h['paid_date']=='0000-00-00'){
                    if(!isset($total_unpaid[$subscription['currency_id']])) $total_unpaid[$subscription['currency_id']] = 0;
