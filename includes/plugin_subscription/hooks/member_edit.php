    ...
    <tr>
        ...
        <td>
            ...
            foreach($sorted_subscriptions as $subscription){
                ...
                ?>
                <div class="subscription<?php echo isset($members_subscriptions[$subscription['subscription_id']])?' active':'';?>" <?php if(module_subscription::can_i('edit','Subscriptions')){ ?> data-settings-url="<?php echo module_subscription::link_open($subscription['subscription_id'],false,$subscription);?>" data-settings-class="data-settings-right" <?php } ?>>
                <input type="checkbox" name="subscription[<?php echo $subscription['subscription_id'];?>]" value="1" id="subscription_<?php echo $subscription['subscription_id'];?>" <?php if(isset($members_subscriptions[$subscription['subscription_id']])) echo 'checked';?> onchange="$(this).parent().find('.subscription_start_date').show();">
                    <label for="subscription_<?php echo $subscription['subscription_id'];?>"><?php echo htmlspecialchars($subscription['name']);?></label> - <?php echo dollar($subscription['amount'],true,$subscription['currency_id']);
                ...
                if(isset($members_subscriptions[$subscription['subscription_id']])){
                    ...
                    if(module_config::c('subscription_allow_credit',1)){
                        ?>
                        <input type="checkbox" value="1" name="subscription_credit[<?php echo $subscription['subscription_id'];?>]" <?php echo isset($members_subscriptions[$subscription['subscription_id']]) && $members_subscriptions[$subscription['subscription_id']]['use_as_credit_bucket'] ? ' checked' : '';?>> <?php _e('Add these payments to credit that can be used for invoice payments.');?><br/>
                        <?php
                        if(isset($members_subscriptions[$subscription['subscription_id']]) && $members_subscriptions[$subscription['subscription_id']]['use_as_credit_bucket']){
                            ...
                            ?>
                            <ul>
                                <li>Total Amount Received: <strong><?php echo dollar($subscription_credit[$subscription['subscription_id']]['total']);?></strong></li>
                                <li>Total Used As Invoice Credit: <strong><?php echo dollar($subscription_credit[$subscription['subscription_id']]['used']);?></strong>
                                    <?php if(count($subscription_credit[$subscription['subscription_id']]['paid_invoices'])){ ?>
                                        (<?php foreach($subscription_credit[$subscription['subscription_id']]['paid_invoices'] as $invoice_id){ echo module_invoice::link_open($invoice_id,true) .' '; } ?>)
                                    <?php } ?>
                                </li>
                                <li>Remaining Amount Of Credit: <strong><?php echo dollar($subscription_credit[$subscription['subscription_id']]['remain']);?></strong></li>
                            </ul>
                            <?php
                        }
                    }
                    ...
                    if(module_config::c('subscription_allow_limits',1)){
                        _e('Maximum renewals:');?> <input type="text" value="<?php echo isset($members_subscriptions[$subscription['subscription_id']]['recur_limit']) && $members_subscriptions[$subscription['subscription_id']]['recur_limit'] > 0 ? (int)$members_subscriptions[$subscription['subscription_id']]['recur_limit'] : '';?>" name="subscription_recur_limits[<?php echo $subscription['subscription_id'];?>]" style="width:21px">
                ...
                if(isset($members_subscriptions[$subscription['subscription_id']]) && module_security::is_page_editable() && $remaining_invoices != 0){
                    ...
                    if(isset($subscription['automatic_renew'])&&$subscription['automatic_renew']){
                        ...
                        <?php _e('dated');?> <input type="text" name="foo" id="date_<?php echo $subscription['subscription_id'];?>" value="<?php echo print_date($next_due_time);?>" class="date_field">
