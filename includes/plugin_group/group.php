...
class module_group extends module_base{
            ...
            foreach($_REQUEST['group_module'] as $owner_table => $groups){
                foreach($groups as $group_id => $owner_id){
                    if($group_id=='new'){
                        ...
                        if(!$group_id||$group_id=='new')continue;
                    }
    ...
    public static function link_generate($group_id=false,$options=array(),$link_options=array()){
        ...
        if(!isset($options['type']))$options['type']='group';
                ...
                <tbody>
                    ...
                    foreach($group_items as $group_item){
                        ...
                        <tr id="group_<?php echo $group_id;?>">
                            <th class="width1"<?php if(self::can_i('edit','Groups')){ ?> data-settings-url="<?php echo module_group::link_open($group_id,false);?>" <?php } ?>>
                                <?php if($can_edit){ ?>
                                <input type="hidden" name="group_module[<?php echo $owner_table;?>][<?php echo $group_id;?>]" value="<?php echo htmlspecialchars($owner_id);?>">
                                <input type="checkbox" name="used_group_module[<?php echo $owner_table;?>][<?php echo $group_id;?>]" id="groupchk<?php echo $owner_table.$group_id;?>" value="<?php echo htmlspecialchars($owner_id);?>" <?php echo isset($group_item['checked']) && $group_item['checked'] ? ' checked':''; ?>>
                                <?php }else{ ?>
                                    <input type="checkbox" name="group" value="<?php echo htmlspecialchars($owner_id);?>" <?php echo isset($group_item['checked']) && $group_item['checked'] ? ' checked':''; ?> disabled="">
                                <?php } ?>
                            </th>
                            <td>
                                <label for="groupchk<?php echo $owner_table.$group_id;?>"><?php echo htmlspecialchars($group_item['name']);?></label>
                            </td>
                        </tr>
                        <?php
                    }
                    ...
                    <tr id="group_<?php echo $group_id;?>">
                        <th class="width1">
                            <input type="hidden" name="group_module[<?php echo $owner_table;?>][<?php echo $group_id;?>]" value="<?php echo htmlspecialchars($owner_id);?>">
                            <input type="checkbox" name="used_group_module[<?php echo $owner_table;?>][<?php echo $group_id;?>]" id="groupchk<?php echo $owner_table.$group_id;?>" value="<?php echo htmlspecialchars($owner_id);?>">
                        </th>
                        <td>
                            <label for="groupchk<?php echo $owner_table.$group_id;?>"></label>
                            <input type="text" name="group_module_name[<?php echo $owner_table;?>][<?php echo $group_id;?>]" autocomplete="off" placeholder="<?php _e('New Group');?>" onkeyup="$('#groupchk<?php echo $owner_table.$group_id;?>').prop('checked',true);">
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
	...
	public static function get_group($group_id){
        if(isset(self::$group_cache[$group_id]))return self::$group_cache[$group_id];
    ...
    public static function newsletter_callback($args){
        if(!isset($args['owner_table']) || !$args['owner_table'])return array();
...
//                    $user['last_name'] = implode(' ',$name_parts);
    ...
    public static function display_pagination_hook(){
        ...
        if(isset(self::$pagination_options['fields'])){
            ...
            ?>
            <span>
            <a href="#" onclick="if($('#group_popdown').css('display')=='inline' || $('#group_popdown').css('display')=='block') $('#group_popdown').css('display','none'); else $('#group_popdown').css('display','inline'); return false;"><?php _e('(group)');?></a>
            <span id="group_popdown" style="position: absolute; width: 200px; display: none; background: #EFEFEF; margin-left: -210px; margin-top: 30px; border: 1px solid #CCC; text-align: left; padding: 6px; z-index: 3;">
                ...
                if(!count($groups)){
                    ...
                    <input type="button" name="add_group_button" id="add_group_button" value="<?php _e('Add to group');?>">
        ...
        if(isset(self::$pagination_options['bulk_actions']) && count(self::$pagination_options['bulk_actions'])){
            ...
            ?>
        <span>
            <a href="#" onclick="if($('#bulk_popdown').css('display')=='inline' || $('#bulk_popdown').css('display')=='block') $('#bulk_popdown').css('display','none'); else $('#bulk_popdown').css('display','inline'); return false;"><?php _e('(bulk actions)');?></a>
                ...
                <?php
                    foreach(self::$pagination_options['bulk_actions'] as $bulk_action_id => $bulk_action_data){
                        switch($bulk_action_data['type']){
                            case 'delete':
                                ?>
                                    <input type="checkbox" class="bulk_action" name="bulk_action[<?php echo $bulk_action_id;?>]" id="bulkchk<?php echo $bulk_action_id;?>" value="yes">
                                    <label for="bulkchk<?php echo $bulk_action_id;?>"><?php _e($bulk_action_data['label']);?></label> <br/>
                                <?php
                                break;
                            case 'form':
                                ?>
                                    <input type="checkbox" class="bulk_action" name="bulk_action[<?php echo $bulk_action_id;?>]" id="bulkchk<?php echo $bulk_action_id;?>" value="yes">
                                    <label for="bulkchk<?php echo $bulk_action_id;?>"><?php _e($bulk_action_data['label']);?></label>
                    ...
                    <input type="button" name="bulk_action_button" id="bulk_action_button" value="<?php _e('Perform Bulk Actions');?>">
                    <script type="text/javascript">
                        $(function(){
