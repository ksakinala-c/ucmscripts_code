...
<form action="" method="post" id="invoice_form">
    ...
    foreach($invoice['taxes'] as $id=>$tax){  ?>
        <div class="dynamic_block">
            <input type="hidden" name="tax_ids[]" class="dynamic_clear" value="<?php echo isset($tax['invoice_tax_id']) ? (int)$tax['invoice_tax_id'] : 0;?>">
		    ...
		    function () use ( &$invoice ) {
			    if(!$invoice['customer_id'] || isset($_REQUEST['change_customer'])){
				    ...
				    if(module_invoice::can_i('edit','Invoices')){
					    ?> <a href="<?php echo module_invoice::link_open($invoice['invoice_id']);?>&change_customer"><?php _e('(change)');?></a> <?php
				    }
                }
		    },
	    ),
        ...
        ?>
            ...
            <div class="tableclass_form content">
                ...
                <p style="text-align: center;">
                    <input type="submit" name="butt_email" id="butt_email2" value="<?php echo _l('Email Invoice'); ?>" class="submit_button btn" />
                    <?php _h('Click this button to email the invoice to the customer from the system'); ?>
                </p>
                <p style="text-align: center;">

                    <button type="button" name="buttmarksent" class="submit_button no_permissions btn" onclick="$('#date_sent').val('<?php echo print_date(date('Y-m-d'));?>'); $('#invoice_form')[0].submit(); return false;"><?php _e('Mark invoice as sent');?></button>
        ...
        <table class="tableclass tableclass_form tableclass_full" cellpadding="0" cellspacing="0">
                ...
                <td>
                    ...
                    foreach($payment_methods as $payment_method){
                        if($payment_method->is_method('online') && $payment_method->is_enabled() && $payment_method->is_allowed_for_invoice($invoice_id)){ ?>
                            <input type="radio" name="payment_method" value="<?php echo $payment_method->module_name;?>" id="paymethod<?php echo $x;?>" class="no_permissions payment_method payment_method_online" <?php echo $default_payment_method==$payment_method->module_name ? 'checked':'';?>>
                    ...
                    foreach($payment_methods as $payment_method){
                        if($payment_method->is_method('offline') && $payment_method->is_enabled() && $payment_method->is_allowed_for_invoice($invoice_id)){ ?>
                            <input type="radio" name="payment_method" value="<?php echo $payment_method->module_name;?>" id="paymethod<?php echo $x;?>" class="no_permissions payment_method payment_method_offline" <?php echo $default_payment_method==$payment_method->module_name ? 'checked':'';?>>
                            <label for="paymethod<?php echo $x;?>"><?php echo $payment_method->get_payment_method_name(); ?></label> <br/>
                            <input type="hidden" id="text_paymethod<?php echo $x;?>" value="<?php echo htmlspecialchars($payment_method->get_invoice_payment_description($invoice_id));?>">
                            <?php
                            $x++;
                        }
                    }
            ...
            <tr class="payment_type_online">
                <td align="center" colspan="2">
                    ...
                    <button type="button" name="buttpay" class="submit_button no_permissions btn btn-primary" onclick="$('#butt_makepayment').val('yes'); this.form.submit();"><?php _e('Make Payment');?></button>
                </td>
            </tr>
        ...
        unset($fieldset_data);
         ...
         ?>
            <div class="tableclass_form content">
                ...
                <?php }else{ ?>
                    <p align="center"><?php _e('This invoice is overdue, but it has not been sent yet. Please pick a "Sent Date" or click below to send invoice.');?></p>
                    <p align="center">
                    <input type="submit" name="butt_email" id="butt_email_overdue" value="<?php echo _l('Email Invoice'); ?>" class="submit_button btn" />
                    </p>
                <?php } ?>
            </div>
        <?php
    ...
    if (isset($invoice['deposit_job_id']) && $invoice['deposit_job_id'] > 0){
        ...
        unset($fieldset_data);
                ...
                if((int)$invoice_id>0 && module_invoice::can_i('edit','Invoices') && $invoice['total_amount_due'] > 0 && $customer_data && ( count($subscription_credits) || $customer_data['credit']>0 ) && (!$invoice['date_cancel']||$invoice['date_cancel']=='0000-00-00')){
                    ...
                    <div class="tableclass_form content">
                        <?php if(count($subscription_credits)){
                            ?>
                            <input type="hidden" name="apply_credit_from_subscription_bucket" id="apply_credit_from_subscription_bucket" value="0">
                            ...
                            foreach($subscription_credits as $subscription_id => $subscription_credit){
                                ...
                                <p align="center">
                                    ...
                                    <br/>
                                    <a href="#"  onclick="$('#apply_credit_from_subscription_bucket').val('do'); $('#apply_credit_from_subscription_id').val('<?php echo $subscription_credit['subscription_owner_id'];?>'); $('#invoice_form')[0].submit(); return false;" class="uibutton"><?php _e('Apply %s credit to this invoice',dollar($apply_credit));?></a>
                                </p>
                            <?php }
                            }
                        }
                ...
                ?>
                    ...
                    <table border="0" cellspacing="0" cellpadding="2" id="invoice_task_items" class="tableclass tableclass_rows tableclass_full">
                        <thead>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($invoice['default_task_type']==_TASK_TYPE_AMOUNT_ONLY){
                                    ...
                                    ?>
                                     <input type="text" name="invoice_invoice_item[new][hours]" value="" id="newinvoice_itemqty" size="3" style="width:30px;" onchange="setamount(this.value,'new');" onkeyup="setamount(this.value,'new');">
                                <?php
                                } ?>
                            ...
                            <td align="center">
                                ...
                                <input type="hidden" name="invoice_invoice_item[new][taxable]" id="invoice_taxable_item_new" value="<?php echo module_config::c('task_taxable_default',1) ? 1 : 0;?>">
                                <input type="hidden" name="invoice_invoice_item[new][manual_task_type]" id="manual_task_type_new" value="-1">
                            </td>
                        </tr>
						</tbody>
                        <?php }
                            ...
                            ?>
                                ...
                                <tr class="<?php echo $c++%2 ? 'odd':'even';?>">
                                    ...
                                    <td>
                                        ...
                                        if($long_description != ''){ ?>
                            <a href="#" class="task_toggle_long_description">&raquo;</a>
                            <div class="task_long_description" <?php if(module_config::c('invoice_show_long_desc',1)){ ?> style="display:block;" <?php } ?>><?php echo module_security::purify_html(forum_text(trim($long_description), false));?></div>
                            <?php }else{ ?>
    &nbsp;
    <?php } ?>
                                    </td>
                        ...
                        <tr>
                            <td colspan="<?php echo $colspan;?>" align="center">
                                <a href="<?php echo module_invoice::link_open($invoice_id);?>&_process=assign_credit_to_customer"><?php _e('This customer has overpaid this invoice. Click here to assign this as credit to their account for a future invoice.');?></a>
                            </td>
                            <td>
                                <?php _e('Credit:');?>
                            </td>
                                ...
                                <tr id="invoice_payment_edit_<?php echo $invoice_payment_id;?>" style="display:none;">
                                    ...
                                    <td style="white-space: nowrap">
	                                    ...
	                                    <?php if(class_exists('module_finance',false) && module_finance::is_plugin_enabled() && module_finance::can_i('view','Finance') && module_finance::is_enabled()){
		                                    ...
		                                    if($existing_finance){
			                                    ?> <a href="<?php echo module_finance::link_open($existing_finance['finance_id']);?>"><?php _e('More');?></a> | <?php
		                                    }else{
			                                    ?> <a href="<?php echo module_finance::link_open( 'new', false ) .'&invoice_payment_id='.$invoice_payment_data['invoice_payment_id'];?>"><?php _e('More');?></a> | <?php
		                                    }
	                                    }
	                                    ?>
                                        <a href="<?php echo module_invoice::link_receipt($invoice_payment_data['invoice_payment_id']);?>" target="_blank"><?php _e('Receipt');?></a>
                                    </td>
                                </tr>
                            <?php } ?>
                                <tr id="invoice_payment_preview_<?php echo $invoice_payment_id;?>">
                                    <td>
                                        ...
                                        if($invoice_payment_data['date_paid'] == '0000-00-00'){
                                            ?> (<a href="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']).'&_process=make_payment&invoice_payment_id='.$invoice_payment_id;?>"><?php _e('complete');?></a>) <?php
                                        }
                                        ?>
                                    </td>
                                    <td>
