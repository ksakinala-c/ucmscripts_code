                ...
                if($this->can_i('edit','Invoices') && module_config::c('invoice_alerts',1)){
                    ...
                    if($cached_alerts = module_cache::get('invoice',$cache_key.$key)){
                        ...
                        foreach($invoices as $invoice){
                            ...
                            if(!$invoice||$invoice['invoice_id']!=$invoice['invoice_id'])continue;
...
','Used when displaying the external view of a credit note.','code');
                        ...
                        if(isset($invoice['credit_note_id']) && $invoice['credit_note_id']){
	                        ...
	                        $template = false;
	                        $invoice_template = isset($invoice['invoice_template_print']) && strlen($invoice['invoice_template_print']) ? $invoice['invoice_template_print'] : module_config::c('invoice_template_print_default','invoice_print');
							...
							if(isset($invoice_template_suffix) && strlen($invoice_template_suffix) > 0){
								$template = module_template::get_template_by_key('external_invoice'.$invoice_template_suffix);
...
<li><span style="font-size: small;">Payment Date:&nbsp;<strong>{PAYMENT_DATE}</strong></span></li>
    ...
    public static function get_replace_fields($invoice_id,$invoice_data=false){
	    ...
	    if(!is_array($customer_data))$customer_data=array();
    ...
    //            print_r($invoice);exit;
        ...
        if(!$invoice || !is_array($invoice) || !isset($invoice['invoice_id']) || !$invoice['invoice_id']){
            ...
            $invoice = array(
                ...
                'discount_type' => $job_id > 0 && isset($job_data['discount_type']) ? $job_data['discount_type'] : module_config::c('invoice_discount_type',_DISCOUNT_TYPE_BEFORE_TAX), // 1 = After Tax
                ...
                'renew_email' => 0,
                'overdue' => false,
	            'invoice_template_print' => '', // module_config::c('invoice_template_print_default','invoice_print')
	            'website_id' => '0',
	            'website_ids' => '',
            );
...
//                $invoice['total_sub_amount_taxable']-=$invoice['discount_amount'];
        ...
        if($invoice_id){
            ...
            if(isset($data['invoice_invoice_item']) && is_array($data['invoice_invoice_item'])){
                foreach($data['invoice_invoice_item'] as $invoice_item_id => $invoice_item_data){
	                ...
	                if(!strlen($invoice_item_data['hours']))$invoice_item_data['hours']=0;
                    $invoice_item_data['hourly_rate'] = number_in($invoice_item_data['hourly_rate'],module_config::c('task_amount_decimal_places',-1));
                    $invoice_item_data['hours'] = number_in($invoice_item_data['hours']);
    ...
    public static function get_finance_recurring_items($hook,$search){
        ...
        foreach($invoices as $invoice){
            ...
            $return[] = array(
                'next_due_date' => date('Y-m-d',strtotime('+'.module_config::c('invoice_due_days',30).' days',strtotime($invoice['date_renew']))),
                'url' => module_invoice::link_open($invoice['invoice_id'],true,$invoice),
                'type' => 'i',
    ...
    public function run_cron($debug=false){
        ...
        $invoice_items = qa($sql);
        if($debug)echo "Processing ".count($invoice_items)." overdue invoices:  <br>\n";
        foreach($invoice_items as $invoice_item){
            module_cache::clear('invoice');
            $invoice = module_invoice::get_invoice($invoice_item['invoice_id']);
            if($invoice['overdue'] && $invoice['overdue_email_auto']){
	            if($debug)echo "Processing overdue for invoice: ".module_invoice::link_open($invoice['invoice_id'],true) ." <br>\n";
                ...
                if($invoice['date_sent'] && $invoice['date_sent']!='0000-00-00' && strtotime($invoice['date_sent']) > strtotime($invoice['date_due'])){
                    ...
                    $invoice_is_due = strtotime($invoice['date_due']);
                    $send_email_on = strtotime('+'.module_config::c('overdue_email_auto_days',3).' days',$invoice_is_due);
	                if($debug)echo module_config::c('overdue_email_auto_days',3)." days from ".$invoice['date_due']." is ".date('Y-m-d',$send_email_on)."<br>\n";
                }else{
                    ...
                    if($debug)echo " - NOT Sending Overdue Invoice Notice for ".module_invoice::link_open($invoice['invoice_id'],true) ." because it has no SENT DATE.<br>\n";
	                $send_email_on = false;
                }
	            if($invoice['date_sent'] && $invoice['date_sent']!='0000-00-00' && date('Y-m-d',$send_email_on) == $invoice['date_sent']){
	                if($debug)echo " - NOT Sending Overdue Invoice Notice for ".module_invoice::link_open($invoice['invoice_id'],true) ." because it was last sent today already.<br>\n";
		            $send_email_on = false;
                }
                ...
                if($send_email_on !== false && $send_email_on <= strtotime(date('Y-m-d'))){
                    if($debug)echo " - Automatically Sending Overdue Invoice Notice for ".module_invoice::link_open($invoice['invoice_id'],true) ."<br>\n";
                    if($debug)echo " - Emailing invoice to customer...";
                    if(module_invoice::email_invoice_to_customer($invoice['invoice_id'],$debug)){
                        if($debug)echo "sent successfully<br>\n";
                    }else{
        ...
        foreach($renew_invoices as $renew_invoice){
            ...
            if($debug)echo "Automatically Renewing invoice ".module_invoice::link_open($renew_invoice['invoice_id'],true) ."<br>\n";
            ...
            if(module_config::c('invoice_auto_renew_only_paid_invoices',1) && $invoice_data['total_amount_due'] > 0){
                ...
                if($debug)echo "NOT RENEWING INVOICE because it hasn't been paid yet !!! <br>\n";
            }else{
                $new_invoice_id = $this->renew_invoice($renew_invoice['invoice_id']);
                if($new_invoice_id){
                    //module_cache::clear_cache();
                    if($debug)echo "invoice Automatically Renewed: ".module_invoice::link_open($new_invoice_id,true)."<br>\n";
