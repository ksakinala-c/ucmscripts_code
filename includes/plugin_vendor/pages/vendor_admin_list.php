...
<div class="" style="padding: 20px; font-weight:bold;">Notice: This Vendors section will be merged into the core "Customers" area soon, so we can make better use of existing features (such as invoicing). Please go to Settings > Customers > Customer Types and create a new type called "Vendors" and create any new vendors there.</div>
        ...
        $columns['company_name'] = array(
            ...
            'callback' => function($vendor){
                if(isset($vendor['company_ids']) && is_array($vendor['company_ids']) && count($vendor['company_ids'])){
                    ...
                    <a href="<?php echo module_vendor::link_open($vendor['vendor_id'],false);?>"><?php echo htmlspecialchars($company_name);?></a>
                    <?php }
                }else{
                    _e('N/A');
                }
