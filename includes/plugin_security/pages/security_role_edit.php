...
<form action="" method="post">
    ...
    <em>Items will appear above only after that permission is attemped to be accessed. Please create a new user, assign them to this role and login as that user to generate more available permissions above.</em>
	...
	<p><em>Group Permissions:</em> Experimental: this is a new feature we are trying to allow permissions based on "Groups". Please set the "Data Access" dropdown above to "Only my groups" in order for these permissions to work. More groups (e.g. customers) will be added shortly after testing is complete on tickets.</p>
	<table class="tableclass tableclass_rows tableclass_full">
		<thead>
		<tr>
			...
			<tr>
				...
				<td>
					...
					foreach($these_groups as $this_group){
						?>
						<input type="checkbox" id="group_<?php echo $this_group['group_id'];?>" name="group_access[<?php echo $this_group['group_id'];?>]" value="1"<?php echo isset($enabled_groups[$this_group['group_id']]) ? ' checked':'';?>>
						<label for="group_<?php echo $this_group['group_id'];?>"><?php echo htmlspecialchars($this_group['name']);?></label> <br/>
						<?php
					}
					?>
				</td>
