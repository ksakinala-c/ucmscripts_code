...
class module_security extends module_base{
	...
	public static function can_access_data($table_name,&$data,$data_id=false){
		...
		if(self::is_super_admin())return true; // super admin can access everything.
    ...
    public static function get_customer_restrictions(){
        ...
        if(!self::is_logged_in())return array(-1=>-1);
        /*
	...
	public static function process_password_reset(){
            ...
            foreach($users as $user){
                ...
                if(strtolower($user['email']) == $email){
                    ...
                    if($email->send()){
                        ...
                        /// log err?
                        echo 'failed to send email, sorry';exit;
                    }
                }
            }
        }
        if($success || !module_config::c('password_reset_debug',0)){
            ...
            echo 'No users found matching '.htmlspecialchars($email); exit;
        }
	...
	public static function process_login($redirect=true,$captcha_check=true){
		...
		if(!empty($_REQUEST['email']) && !empty($_REQUEST['password'])) {
			...
			if ( strlen( $email ) && strlen( $password ) ) {
				...
				$sql = "SELECT * FROM `" . _DB_PREFIX . "user` WHERE `email` LIKE '" . db_escape( $email ) . "' AND ( `password` = '" . db_escape( $password ) . "' OR `password` = '" . db_escape( md5( $password ) ) . "' )";
				$res = qa1( $sql );
	...
	public function process(){
        ...
        if('save_security_role' == $_REQUEST['_process']){
			...
			if(!module_security::can_i('edit','Security Roles','Security'))redirect_browser('/');
