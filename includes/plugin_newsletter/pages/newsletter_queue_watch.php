            ...
            <td width="30%" valign="top">
                ...
                <table class="tableclass tableclass_form tableclass_full">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($start_time <= time()) _e('Now');
                            else echo print_date($start_time,true); ?>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td id="total_link_clicks">
                            ...
                            <?php if($statistics){ ?>
                            <a href="<?php echo module_newsletter::link_statistics_link_clicks($newsletter_id,$send_id,false,$send);?>"><?php _e('View Link Clicks');?></a>
                            <?php } ?>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td id="failures">
                            ...
                            <?php if(isset($statistics)&&$statistics&&(int)$send['total_fail_count']>0 && !isset($_REQUEST['retry_failures'])){ ?>
                                <a href="<?php echo module_newsletter::link_queue_watch($newsletter_id,$send_id);?>&retry_failures=1"><?php _e('Retry failed emails');?></a>
                            <?php } ?>
                        </td>
                    </tr>
                    <tr>
                            ...
                            switch($send['status']){
                                ...
                                case _NEWSLETTER_STATUS_PAUSED:
                                    _e('Paused');
                                        ?>
                                        <br>
                                        <input type="button" name="delete" value="<?php echo _l('Delete Send');?>" class="submit_button delete_button" onclick="$('#status').val('delete'); $(this).parents('form')[0].submit();">
                                        <?php
                                    break;
                                case _NEWSLETTER_STATUS_PENDING:
                    ...
                    <tr id="change_status_buttons">
                        <td colspan="2" align="center">
                            ...
                            <?php if($send['status']== _NEWSLETTER_STATUS_PENDING){
                                ...
                                ?>
                                <input type="button" name="cancel" value="<?php echo _l('Pause Sending');?>" class="submit_button" onclick="$('#status').val(<?php echo _NEWSLETTER_STATUS_PAUSED;?>); $(this).parents('form')[0].submit();">
                            <?php }else if($send['status']==_NEWSLETTER_STATUS_NEW){
                                // sending hasn't started yet - offer a start button.
                                ?>
                                <input type="button" name="resume" value="<?php echo _l('Start Sending');?>" class="submit_button" onclick="$('#status').val(<?php echo _NEWSLETTER_STATUS_PENDING;?>); $(this).parents('form')[0].submit();">
                            <?php }else if($send['status']==_NEWSLETTER_STATUS_PAUSED){
                                // sending paused.. offer a resume button
                                ?>
                                <input type="button" name="cancel" value="<?php echo _l('Resume Sending');?>" class="submit_button" onclick="$('#status').val(<?php echo _NEWSLETTER_STATUS_PENDING;?>); $(this).parents('form')[0].submit();">
                            <?php }else{ ?>
                                <!-- send complete -->
                            <?php } ?>
                        </td>
                ...
                <div id="manual_send_block" style="<?php echo($send['status']!= _NEWSLETTER_STATUS_PENDING && !$retry_failures) ? 'display:none;':'';?>">
                    <?php print_heading(array(
                                'type'=>'h3',
