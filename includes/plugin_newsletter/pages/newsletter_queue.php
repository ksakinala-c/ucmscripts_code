    ...
    <table width="100%" class="tableclass tableclass_full">
        ...
        <tr>
            <td width="70%" valign="top">
                ...
                <table class="tableclass tableclass_rows tableclass_full">
                    ...
                    <tr>
                        ...
                        <?php foreach (array_keys($extra_fields) as $extra_field){
                            if(!isset($display_extra_fields[strtoupper($extra_field)]))continue;
                            ?>
                        ...
                        foreach($send_members_pagination['rows'] as $send_member){
                            ...
                            <tr class="<?php echo $x++%2?'odd':'even';?>">
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo $cache['_edit_link'];?>" class="ui-state-default ui-corner-all ui-icon ui-icon-pencil" title="<?php _e('Edit');?>" target="_blank"><?php _e('Edit');?></a>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php
                ...
                <p align="center">
                    <input type="button" name="cancel" value="<?php _e('Back / Cancel');?>" class="submit_button" onclick="window.location.href='<?php echo module_newsletter::link_send($newsletter_id);?>';">
                    <input type="button" name="cancel" value="<?php _e('Add More Recipients');?>" class="submit_button" onclick="window.location.href='<?php echo module_newsletter::link_send($newsletter_id,false,$send_id);?>';">
                    <?php if($recipient_count > 0){ ?>
                        <input type="submit" name="send" value="<?php _e('Queue Newsletter for Sending');?>" class="save_button submit_button">
                    <?php } ?>
                </p>
            </form>
                            ...
                            if($x>4){
                                foreach (array_keys($extra_fields) as $extra_field){
                                    if(strtoupper($extra_field)== $key){ ?>
                                           <input type="checkbox" name="f[]" value="<?php echo htmlspecialchars($key);?>" <?php echo isset($display_extra_fields[$key]) ? 'checked':'';?>>
                                    <?php }
                                }
                            }
                            ?>
