...
if(!$search && isset($_REQUEST['do_last_search']) && isset($_SESSION['ticket_last_search']))$search = $_SESSION['ticket_last_search'];
...
$columns['ticket_time'] = array(
    ...
    'callback' => function($ticket){
        if($ticket['last_message_timestamp']>0){
            if($ticket['last_message_timestamp'] < $ticket['limit_time']){
            ...
            if($ticket['last_message_timestamp']>=$ticket['today']){
                echo '<span class="success_text">';
                _e('(today)');
                echo '</span>';
            }else{
                $days = ceil(($ticket['today'] - $ticket['last_message_timestamp'])/86400);
