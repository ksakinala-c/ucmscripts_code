...
class module_template extends module_base{
    ...
    public static function link_open_popup($template_name){
        ...
        if(!$template || !isset($template->template_id) || !$template->template_id)return;
        ...
        if(!$template || (!$template['content'] && $content)){
            ...
            if($tags && isset($template['tags']) && $template_key){
                ...
                foreach($tags as $key=>$val){
                    if(!isset($existing_tags[strtoupper($key)]) && !isset($existing_tags[$key]))$new_tags=true;
                }
                if($new_tags){
    ...
    public function replace_content(){
        ...
        if(preg_match_all('#\{if:([^\}=]+)([^\}]*)\}(.*){endif:\1\}.*#imsU',$content,$matches)){
            foreach($matches[1] as $key => $template_tag){
	            ...
	            $new_content = false;
	            if($debug)echo $matches[0][$key] ."\n";
	            if($debug)print_r($bits);
	            if($debug)print_r($elseif_matches);
	            foreach($elseif_matches[0] as $elseif_key => $elseif_condition){
		            ...
		            if(!strlen(trim($elseif_matches[3][$elseif_key]))){
			            ...
			            if($debug)echo "Reached final {else} statement, using this value\n";
			            $new_content = $bits[$elseif_key];
			            break;
		            }else{
			            if($debug)echo "Checking $elseif_condition against ".$elseif_matches[3][$elseif_key]." which has the value: ".$template_tag_value."\n";
	            ...
	            if($debug)echo "Final content to use will be: \n".$new_content;
