...
<table class="tableclass tableclass_form tableclass_full">
    ...
    <tr>
        <th> </th>
        <td>
	        <a href="#" onclick="if(confirm('Really restore default template?')){ <?php if(isset($template['wysiwyg']) && $template['wysiwyg']){ ?> $('#template_content').tinymce().remove(); <?php } ?> $('#template_content').val(''); $('#template_form')[0].submit(); } return false;">Restore Default</a>
	        <ul>
		        <li><?php _e('Special Template Tags:');?></li>
		        <li><strong>{currency:XXXX}</strong> - <?php _e('This will format XXXX as a currency value, replace XXXX with a template tag below.');?></li>
		        <li><strong>{XXXX+123} or {XXXX-123}</strong> - <?php _e('This will perform basic arithmetic (plus, minus or multiplication) on the XXXX template tag.');?></li>
		        <li><strong>{currency:XXXX*0.5}</strong> - <?php _e('This will show 50% of the XXXX template tag as a currency value.');?></li>
		        <li><strong>{l:Some Word}</strong> - <?php _e('This will display the translation for "Some Word".');?></li>
		        <li><strong>{if:XXXX} 123 {else} 456 {endif:XXXX}</strong> - <?php _e('If the template tag XXXX contains a value then 123 will be displayed, otherwise 456 will be displayed.');?></li>
		        <li><strong>{if:XXXX=Some String} 123 {elseif:XXXX=Another String} 456 {else} 789 {endif:XXXX}</strong> - <?php _e('If the template tag XXXX equals "Some String" then 123 will be displayed, if XXXX equals "Another String" then 456 will be displayed, otherwise 789 will be displayed.');?></li>
		        <li><strong>{XXXX+4d} or {XXXX+2m}</strong> - <?php _e('If XXXX is a date field, perform basic date arithmetic such as plus 4 days or plus 2 months.');?></li>
		        <li><strong>{XXXX+4d} or {XXXX+2m}</strong> - <?php _e('If XXXX is a date field, perform basic date arithmetic such as plus 4 days or plus 2 months.');?></li>
		        <li><strong>{XXXX-m}</strong> - <?php _e('If XXXX is a date field, only output the m/month portion of it. Possible values: ymdYMDjlSWFn');?></li>
		        <li><strong>{DAY} or {MONTH} or {YEAR}</strong> - <?php _e('Output the current day, month or year.');?></li>
            <?php
