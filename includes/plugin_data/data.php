					...
					if($data_field['required']){
						...
						switch($data_field['field_type']){
							...
							case 'file':
								...
								if(!is_array($previous_files)) $previous_files = array();
		...
		foreach($allowed_to_save as $data_field_id => $tf){
			...
			switch($data_field['field_type']){
				...
				case 'file':
					...
					$previous_files = isset($previous_data_items[$data_field_id]['data_text']) ? @unserialize($previous_data_items[$data_field_id]['data_text']) : array();
					if(!is_array($previous_files)) $previous_files = array();
					if(isset($previous_files['name']))$previous_files = array($previous_files);
					...
					foreach($_FILES['data_field']['tmp_name'][$data_field_id] as $file_upload_id => $file_tmp_name) {
						if ( is_uploaded_file( $file_tmp_name ) && isset($_FILES['data_field']['name'][ $data_field_id ][$file_upload_id])) {
							...
							if ( strlen( $user_file ) ) {
								...
								if ( move_uploaded_file( $file_tmp_name, 'includes/plugin_data/upload/' . $file_name ) ) {
									// upload success.
				...
				switch($element['type']){
					...
					case 'file':
						if($value){
							...
							$file_link = 'includes/plugin_data/upload/'.basename($file_data['file']);
									...
									switch(strtolower($matches[1])){
										...
										case 'png':
											if(module_config::c('data_show_image_preview',1)) {
												...
												?>
												<div class="file_preview" style="float:left; width:<?php echo $thumb_width+10;?>px; margin:3px; border:1px solid #CCC; text-align:center;">
	                    ...
	                    if(isset($element['values']) && count($element['values'])){
		                    ...
		                    if($value_temp){
			                    if(!empty($value_temp['file'])) {
				                    $file_link = 'includes/plugin_data/upload/' . basename( $value_temp['file'] );
				                    if ( is_file( $file_link ) ) {
