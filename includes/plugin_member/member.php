...
<form action="" method="post">
    ...
    public static function get_newsletter_recipient($member_id, $basic=false) {
        ...
        if(!$member || !(int)$member['member_id'])return false; // member doesn't exist any more
        if($basic)return $member;
        ...
        $extras = module_extra::get_extras(array('owner_table'=>'member','owner_id'=>$member['member_id']));
                    ...
                    if(class_exists('module_newsletter',false)){
                        ...
                        foreach($group_items as $group_item){
                            ...
                            <div class="group_select">
                                <input type="checkbox" name="member[group][<?php echo $group_item['group_id'];?>]" value="1"<?php foreach($newsletter_subscriptions as $newsletter_subscription){
