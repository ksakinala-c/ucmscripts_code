...
//		return get_multiple("user",$search,"user_id","fuzzy","name");
	...
	public static function print_contact_summary($user_id,$output='html',$fields=array()) {
        ...
        if(!$user['customer_id']&&!$user['vendor_id'])$user = array(); //  only do this for customer contact details.
...
//        echo $sql;
    ...
    public static function is_staff_member($user_id) {
        ...
        if($user_id>0){
            ...
            if(isset($staff_members[$user_id]))return true;
        }
        return false;
    }
    private static $_staff_members_cache = false;
    public static function get_staff_members() {
        if(is_array(self::$_staff_members_cache))return self::$_staff_members_cache;
        ...
        if(!is_array($staff_users))$staff_users = array();
        $staff_contacts = self::get_contacts(array('is_staff'=>1));
        if(!is_array($staff_contacts))$staff_contacts = array();
    ...
    public static function hook_filter_var_contact_list($call, $attributes){
        ...
        foreach(module_user::get_contacts(array(
    ...
    public static function api_filter_user($hook, $response, $endpoint, $method){
        ...
        switch($method) {
            case 'get':
                ...
                foreach(array('user_id','customer_id','name','last_name','email','phone','mobile','fax') as $field){
